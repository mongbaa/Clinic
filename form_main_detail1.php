...
<input type="text" name="form_detail_order" class="form-control" id="form_detail_order" value="<?php echo $form_detail_order;?>" placeholder=" ลำดับ " required>
                      ...
                      <div align="center">
                          ...
                          <select name="weight_type_id" class="form-control" required>
                              ...
                              $query_weight_type = $conn->query($sql_weight_type); 
                               ...
                               <option value="<?php echo $result_weight_type['weight_type_id'];?> " 
                               <?php if($row_form_detail['weight_type_id']==$result_weight_type['weight_type_id']) echo "selected";?> >
                      ...
                      <select name="score_type_id" class="form-control" required>
                       ...
                       ?>
                       <option value="<?php echo $result_score_type['score_type_id'];?> "
                       <?php if($row_form_detail['score_type_id']==$result_score_type['score_type_id']) echo "selected";?>>
				      ...
				      <?php
                      include"config.inc.php";
                      $sql_score = "select  *  from tbl_score  where score_id = 1 ";
                      $query_score = $conn->query($sql_score); 
                      while($result_score = $query_score->fetch_assoc()) {
                      ?>

                      <option value="<?php echo $result_score['score_id'];?> "
                      <?php if($row_form_detail['score_id']==$result_score['score_id']) echo "selected";?>>
                      ...
                      <td Width="10%"><div align="center"> 
                      <input type="checkbox" name="form_detail_complete" class="form-control" value="Y"  <?php if($row_form_detail['form_detail_complete']=="Y") echo "checked";?> />  </div>
                      </td>
...
?>
                  ...
                  <tr>
                    <td><div align="center"><?php echo $i;?>  (<?php echo $row_form_detail['form_detail_order'];?>)</div></td>
                    <td><?php echo $row_form_detail['form_detail_topic'];?>  (<?php echo $row_form_detail['form_detail_field'];?>)  </td>
                    
                    
                    <td><div align="center">
                    <?php if($row_form_detail['weight_type_id']==1){ echo ""; }?>
                    ...
                    <?php if($row_form_detail['form_detail_extra']=="Y"){?>
                      <input type="text" name="extra" class="form-control"  placeholder="">
                    <?php }?>
...
<?php } ?>
          ...
          <!-- /.card -->
			...
			$table = $row_form_main['form_main_table'];
             ...
             $create .= " $field"."_date DATE NOT NULL , ";  // วันที่บันทึก
			...
			$create .= " last_date DATE NOT NULL , "; // วันที่ล่าสุด
...
<script>
  $(function () {
    ...
    $('#example2').DataTable({
      ...
      "ordering": true,
      "info": true,
      "autoWidth": false,CREATE TABLE db_clinic.tbl_score1 ( score_id INT NOT NULL , score_weight FLOAT NOT NULL , score_grade VARCHAR NOT NULL , score_extra FLOAT NOT NULL , score_competency VARCHAR NOT NULL , score_complete VARCHAR NOT NULL , score_teacher INT NOT NULL , score_date DATE NOT NULL , last_date DATE NOT NULL ) ENGINE = InnoDB;
    });
  });
</script>
