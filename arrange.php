...
<section class="content">
 ...
 <?php 
  ...
  $sql_arrange .= " GROUP BY o.student_id ";
...
<div class="card mb-3">
                <div class="row g-0">
                    ...
                    <div class="col-md-8">
                      <div class="card-body">
                      <p class="card-text">
                     รหัส. <b><?php echo $row_arrange['student_id']; ?></b> <br>
                     นทพ. <?php echo nameStudent::get_student_name($row_arrange['student_id']); ?>
                      </p>
                      </div>
                    </div>
                </div>
               ...
               <?php
                   $student_id = $row_arrange['student_id'];
                   $sql_arrange_st = " SELECT a.* , d.* , p.* , f.* , o.* ";
                   $sql_arrange_st .= " FROM (SELECT * FROM tbl_arrange where arrange_date = '$arrange_date' and arrange_time_type = '$arrange_time_type' and teacher_id = $teacher_id and arrange_type = 0 and arrange_check_eval = 0) AS a ";
                   $sql_arrange_st .= " INNER JOIN tbl_detail as d ON d.detail_id = a.detail_id ";
                   $sql_arrange_st .= " INNER JOIN tbl_plan as p ON p.plan_id = d.plan_id ";
                   $sql_arrange_st .= " INNER JOIN tbl_form_main as f ON f.form_main_id = p.form_main_id ";
                   $sql_arrange_st .= " INNER JOIN (SELECT * FROM tbl_order where student_id = '$student_id') as o ON o.order_id = d.order_id";
                   $sql_arrange_st .= " ORDER BY a.arrange_time ASC ";
                   $query_arrange_st = $conn->query($sql_arrange_st);
                   while ($row_arrange_st = $query_arrange_st->fetch_assoc()) {
               ?>


                    <li class="list-group-item">
                   
                          <a href="evaluate.php?detail_id=<?php echo $row_arrange_st['detail_id']; ?>&plan_id=<?php echo $row_arrange_st['plan_id']; ?>&arrange_date=<?php echo $row_arrange_st['arrange_date']; ?>&arrange_id=<?php echo $row_arrange_st['arrange_id']; ?>">
                                       
                                    
                           <b><?php echo $row_arrange_st['HN']; ?></b> 
                          <?php echo $row_arrange_st['fname']; ?>
                          <?php echo $row_arrange_st['lname']; ?> 

                          <?php if ($row_arrange_st['arrange_record_date'] > $row_arrange_st['arrange_date']) {
                          $arrange_record_date = $row_arrange_st['arrange_record_date'];
                          echo "<br><font size = '2' color='red'> ย้อนหลัง ($arrange_record_date)</font>";
                          } else {
                          echo "";
                          }  
                          ?>
                          ...
                          <font color="black">
                          <?php $form_main_detail = $row_arrange_st['form_main_detail'];
                          $array_form_main_detail = json_decode($form_main_detail, true);
                          ?>
                      <?php echo $row_arrange_st['plan_name']; ?>
                      <?php if (in_array("c", $array_form_main_detail)) { ?> <br> Class : <b> <?php echo nameClass::get_class_name($row_arrange_st['class_id']); ?> </b> <?php } ?>
                      <?php if (in_array("s", $array_form_main_detail)) { ?> Surface : <b> <?php echo $row_arrange_st['detail_surface']; ?> </b> <?php } ?>
                      <?php if (in_array("r", $array_form_main_detail)) { ?> <br> Root : <b> <?php echo $row_arrange_st['detail_root']; ?> </b> <?php } ?>
                      <?php if (in_array("t", $array_form_main_detail)) { ?>
                        <?php
                        $array_detail_tooth = (array)json_decode($row_arrange_st['detail_tooth']);
                        $toothh = "";
