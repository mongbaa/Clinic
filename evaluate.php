...
<section class="content">
	...
	<div class="row">
		<div class="col-md-12">
			...
			$sql_arrange = " SELECT a.* , d.* ,  o.*, p.*";
			$sql_arrange .= " FROM (SELECT * FROM tbl_arrange where arrange_date = '$arrange_date' and  teacher_id = '$arrange_teacher_id' ) AS a ";
			$sql_arrange .= " INNER JOIN tbl_detail as d ON d.detail_id = a.detail_id ";
			$sql_arrange .= " INNER JOIN (SELECT * FROM tbl_order WHERE student_id = $student_id ) as o ON o.order_id = d.order_id";
			...
			while ($row_arrange = $query_arrange->fetch_assoc()) {
				...
				if ($row_arrange['arrange_id'] == $arrange_id) {
				...
				$cla = nameClass::get_class_name($row_arrange['class_id']);
				$surf = $row_arrange['detail_surface'];
				$roo = $row_arrange['detail_root'];
				...
				$data_e = $row_arrange['plan_name']." ".$cla." ".$surf." ".$roo." ".$toothh;
			?>


<?php 
if($row_arrange['arrange_type']==1){  // LAB

	$arrange_time_type = "LAB";

}else{

	$arrange_time_type = $row_arrange['arrange_time_type'];
}

?>

				<a href="evaluate.php?detail_id=<?php echo $row_arrange['detail_id']; ?>&plan_id=<?php echo $row_arrange['plan_id']; ?>&arrange_date=<?php echo $row_arrange['arrange_date']; ?>&arrange_id=<?php echo $row_arrange['arrange_id']; ?>" class="btn btn-<?php echo $bgg;?>" data-toggle="tooltip" title="<?php echo $data_e;?>">
					<?php echo $row_arrange['HN']; ?> :
					<?php echo $row_arrange['fname']; ?>
					<?php echo $row_arrange['lname']; ?>
					(<?php echo $arrange_time_type;?>)

					<?php
					if ($row_arrange['arrange_check_eval'] == 1) { ?>
						<font color = "green">
			...
			<div class="card card-primary">
				...
				<div class="card-body">
					...
					<div class="row">
						...
						<div class="col-md-6 col-sm-6 col-12">
							<div class="info-box">
								<div class="info-box-content">
									...
									<?php if (isset($arrange_detail)) { ?>
										<span class="info-box-text">
											<h5>รายละเอียด :
												<?php include "evaluate_arrange_detail.php"; ?>
											</h5>
										</span>
									<?php } ?>
						...
						<?php
						//อ.ผู้ประเมินก่อนหน้า
						$sql_arrange_last = "SELECT * FROM tbl_arrange  where detail_id = $detail_id and arrange_date <'$arrange_date' and arrange_time_type != '$arrange_time_type' and arrange_check_eval = 1 ";
						$query_arrange_last = $conn->query($sql_arrange_last);
						$row_arrange_last = $query_arrange_last->fetch_assoc();
						if (isset($row_arrange_last['teacher_id']) && !empty($row_arrange_last['teacher_id'])) {
							$arrange_date_last = $row_arrange_last['arrange_date'];
							$teacher_id_last = $row_arrange_last['teacher_id'];
						} else {
							$arrange_date_last = $arrange_date;
							$teacher_id_last = $arrange_teacher_id;
						}
						?>
						...
						<div class="col-md-6 col-sm-6 col-12">
							<div class="info-box">
								<div class="info-box-content">
									<span class="info-box-text"> วันที่ประเมินล่าสุด : <b>
									<?php
									$date_last = date_create($arrange_date_last);
									echo date_format($date_last,"d-m-Y");
									?>
									<?php //echo $arrange_date_last; ?>
								</b> </span>
									<span class="info-box-text"> ผู้ประเมินล่าสุด : <b> <?php echo nameTeacher::get_teacher_name($teacher_id_last); ?> </b> </span>
									<span class="info-box-text">
										...
										<?php if (in_array("c", $array_form_main_detail)) { ?> Class : <b> <?php echo nameClass::get_class_name($row_arrange_detail['class_id']); ?> </b> <?php } ?>
						...
						<div class="row">
							...
							<div class="form-group clearfix">
								<div class="icheck-danger d-inline">
									...
									<font color="red">(**เมื่อติ๊ก Complete หมายถึง การรักษางานนี้เสร็จสิ้น นทพ.จะไม่สามารถส่งงานนี้ได้อีก)</font>
								</div>
							</div>
...
if(isset($_GET['redone']))  { //เมื่อกด  redone ให้สร้าง  record เพิ่ม 
	
	$detail_id = $_GET['detail_id'];
	$plan_id = $_GET['plan_id'];
	$last_date = $_GET['arrange_date'];
	$arrange_id = $_GET['arrange_id'];

	$sql_redone = "INSERT INTO tbl_".$tbl." ($tbl_id , detail_id) VALUES  (NULL, '{$detail_id}')";
	$conn->query($sql_redone); 
	$id_form = $conn->insert_id;	


	echo "<script type='text/javascript'>";
	//echo  "alert('สร้างฟอร์มหลักสำเร็จ');";
	echo "window.location='evaluate.php?detail_id=$detail_id&plan_id=$plan_id&arrange_date=$last_date&arrange_id=$arrange_id&id_form=$id_form'";
	echo "</script>";
