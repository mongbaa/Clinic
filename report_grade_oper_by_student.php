...
<?php
echo "<a href='report_grade_oper_by_student_excel.php?act=excel&level_search=$level_search&student_search=$student_search&date_start=$date_start&date_end=$date_end' class='btn btn-primary'> Export->Excel </a>";
?>
            ...
            ?>
               ...
               // $sql_detail  .=  " INNER JOIN (SELECT * FROM tbl_detail WHERE  plan_id = '$plan_id_oper' and (detail_complete = 1 or detail_date_complete != '0000-00-00')  $detail_work_ids ) as d  ON  d.order_id = o.order_id";
                ...
                $sql_detail  .=  " INNER  JOIN (SELECT * FROM tbl_form_main where form_main_confirm = 1  ) AS f  ON  f.form_main_id = p.form_main_id ";
                 ...
                 $sql_detail  .=  " ORDER BY d.class_id ASC ";
                ...
                ?>
                    ...
                    ?>
                        ...
                        <h4>
                            ...
                            <?php if ($result_detail['detail_complete'] == 1) { //complete 
                            ?>
                        ...
                        <div class="card card-default color-palette-box">
                            <div class="card-header bg-info">
                                ...
                                $difficulty = $row_diagnosis['oper_diagnosis_difficulty'];

                                $dif = json_decode($difficulty);

                               // var_dump(json_decode($difficulty));
                                ...
                                /*
                                if (!empty($difficulty) || isset($difficulty)) {

                                   // var_dump(json_decode($difficulty));
                                   //$df = json_encode($difficulty);
                                  echo $df = json_decode($difficulty);
                                   $df = $df[0];
                                   ...
                                   //echo  $diagnosis_difficulty = json_decode($diagnosis_difficulty);
                                   //var_dump(json_decode($difficulty));
                                ...
                                */
                                ...
                                if ($plan_id_oper == 47) { //เช็คหน่วยงาน Filling  
                                    ...
                                    */
                                    ...
                                    if (!empty($grade_chk_Beginning) && !empty($grade_chk_Restoration) && !empty($grade_chk_Polishing)) {
                                        $chk_unit = 1;
                                        $unit = ($row_class['class_score'] * $chk_unit) + $df;
                                        //ประเมิน 3.1,3.2 = หน่วย *0.75 *Difficulty
                                ...
                                if ($plan_id_oper == 52) { //เช็คหน่วยงาน Direct veneer
                                    ...
                                    if (!empty($grade_chk_Beginning) && !empty($grade_chk_Restoration) && !empty($grade_chk_Polishing)) {
                                   ////ประเมิน 3.1,3.2,3.3 = หน่วย *1 *Difficulty
                                        $unit = ($row_class['class_score'] * 1) + $df;
                                        
                                    } else if (!empty($grade_chk_Beginning) && !empty($grade_chk_Restoration)) {
                                  //ประเมิน 3.1,3.2 = หน่วย *0.75 *Difficulty
                                        $unit = ($row_class['class_score'] * 0.75) + $df;
                                    } else {
